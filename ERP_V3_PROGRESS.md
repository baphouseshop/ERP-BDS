...
- [x] Sửa lỗi RLS và bổ sung Foreign Key cho bảng audit_logs để hiển thị dữ liệu tin cậy.
- [x] Fix bug logic Filter/Search khiến danh sách hiển thị 0 bản ghi khi gặp dữ liệu trống.
- [x] Tích hợp Real-time cho trang lịch sử hệ thống.
- [x] Tự động hóa ghi nhận User, Email và Phòng ban (Role) trong Audit Log.
- [x] Cải tiến RPC 'Xác nhận hợp đồng' để bắt thông tin người thực hiện chính xác hơn.
- [x] Thiết kế UI Audit Log cao cấp với hiệu ứng Glassmorphism và stats bento grid (Vertical Layout).
- [x] Tối ưu hóa giao diện chi tiết thay đổi: Loại bỏ raw JSON, hiển thị phân tích human-readable.
- [x] Xử lý hiển thị thông tin cho các tác vụ tự động (Automation/System).
